...
Page({
  ...
  last() {
    if (this.data.true_num > 0) {
      ...
      wx.setStorageSync('cihuiyufa_10thday_num', this.data.true_num)
      console.log(wx.getStorageSync('cihuiyufa_10thday_num'))
      this.searchSqlEnglish_1()
    }
    else {
      wx.showToast({
  ...
  choice(e) {
    const query = Bmob.Query("cihuiyufa_10thday");
  ...
  searchSqlEnglish_1: function () {
    if (wx.getStorageSync('cihuiyufa_10thday_num') != 0) {
      this.setData({
        true_num: wx.getStorageSync('cihuiyufa_10thday_num')
      })
    }
    ...
    try {
      // 此处是可能产生例外的语句
      const query1 = Bmob.Query("cihuiyufa_10thday");
      const query = Bmob.Query("cihuiyufa_10thday");
      // query1.count().then(res => {
      //   console.log(res)
      //   wx.setStorageSync('cihuiyufa_10thday_length', res)
      // });
      ...
      query.find().then(res => {
        ...
        if (that.data.true_num + 1 <= wx.getStorageSync('cihuiyufa_10thday_length')) {
          if (res[0].str != '') {
            that.setData({
